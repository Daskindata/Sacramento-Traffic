...
rf.model1 <- randomForest(is_double_parked ~., data = trainingData[1:800000,], na.action = na.omit, proximity=FALSE, importance = TRUE)
rf.model1

# Fine tuning parameters of Random Forest model
rf.model2 <- randomForest(is_double_parked ~ ., data = trainingData[1:800000,], ntree = 500, mtry = 6, proximity = FALSE, importance = TRUE, na.action = na.omit)
rf.model2
...
mean(predvalid == testset$is_double_parked)
table(predvalid, testset$is_double_parked)
